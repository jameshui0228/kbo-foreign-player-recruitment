#!/usr/bin/env python3
"""Train pilot KBO translation and failure-risk models.

These models are role-specific and leakage-safe: only pre-KBO Savant features
are allowed as explanatory variables. Outputs are for model validation and
message discovery, not candidate recommendation.
"""
...
MART_PATH = OUTPUT_DIR / "kbo_translation_feature_mart_v0_1.csv"
...
def feature_columns(df: pd.DataFrame, role: str) -> list[str]:
    ...
    if role == "hitter":
        blocked = {"pre_woba_allowed", "pre_starter_stabilizer_score"}
    else:
        blocked = {"pre_woba", "pre_ssg_message_screen_score"}
...
def run_time_folds(mart: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    pred_rows = []
    score_rows = []
    trainable = mart[mart["has_pre_kbo_savant_features"] & mart["label_available"]].copy()
...
def main() -> None:
    ...
    for col in ["has_pre_kbo_savant_features", "label_available"]:
        mart[col] = mart[col].astype(str).str.lower().isin(["true", "1", "1.0"])
    for col in TARGETS:
    ...
    preds.to_csv(OUTPUT_DIR / "kbo_translation_failure_oof_predictions_v0_1.csv", index=False)
    scores.to_csv(OUTPUT_DIR / "kbo_translation_failure_fold_scores_v0_1.csv", index=False)
    comparison.to_csv(OUTPUT_DIR / "kbo_translation_failure_model_comparison_v0_1.csv", index=False)
    repeated_scores.to_csv(OUTPUT_DIR / "kbo_translation_failure_repeated_cv_scores_v0_1.csv", index=False)
    repeated_comparison.to_csv(OUTPUT_DIR / "kbo_translation_failure_repeated_cv_comparison_v0_1.csv", index=False)
    feature_signals.to_csv(OUTPUT_DIR / "kbo_translation_failure_feature_signals_v0_1.csv", index=False)

    print("wrote", OUTPUT_DIR / "kbo_translation_failure_oof_predictions_v0_1.csv", preds.shape)
    print("wrote", OUTPUT_DIR / "kbo_translation_failure_fold_scores_v0_1.csv", scores.shape)
    print("wrote", OUTPUT_DIR / "kbo_translation_failure_model_comparison_v0_1.csv", comparison.shape)
    print("wrote", OUTPUT_DIR / "kbo_translation_failure_repeated_cv_scores_v0_1.csv", repeated_scores.shape)
    print("wrote", OUTPUT_DIR / "kbo_translation_failure_repeated_cv_comparison_v0_1.csv", repeated_comparison.shape)
    print("wrote", OUTPUT_DIR / "kbo_translation_failure_feature_signals_v0_1.csv", feature_signals.shape)
    print()
